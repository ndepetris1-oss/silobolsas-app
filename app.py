...
def panel():
    ...
    silos = conn.execute("""
        SELECT s.*,
        (SELECT id FROM muestreos WHERE numero_qr=s.numero_qr
         ORDER BY fecha_muestreo DESC LIMIT 1) ultimo
        FROM silos s
    ...
    for s in silos:
        grado, factor = None, None
        if s["ultimo"]:
            rows = conn.execute("""
                SELECT seccion, grado, factor FROM analisis
                WHERE id_muestreo=?
            """,(s["ultimo"],)).fetchall()

            if rows:
                factor = round(sum((r["factor"] or 0)*pesos[r["seccion"]] for r in rows)*100,1)
                grado = max([r["grado"] for r in rows if r["grado"] is not None], default=None)
...
def guardar_analisis_seccion():
    ...
    conn = get_db()
    ex = conn.execute("""
        SELECT id FROM analisis WHERE id_muestreo=? AND seccion=?
...
def ver_silo(qr):
    ...
    muestreos = conn.execute("""
        SELECT id, fecha_muestreo FROM muestreos
        WHERE numero_qr=? ORDER BY fecha_muestreo DESC
    """,(qr,)).fetchall()
    conn.close()
...
def ver_muestreo(id):
    ...
    muestreo = conn.execute("""
        ...
        FROM muestreos m JOIN silos s ON s.numero_qr=m.numero_qr
        WHERE m.id=?
    """,(id,)).fetchone()

    analisis = conn.execute("""
        SELECT * FROM analisis WHERE id_muestreo=?
        ORDER BY seccion
    """,(id,)).fetchall()
...
def exportar():
    ...
    rows = conn.execute("""
        SELECT s.numero_qr, s.cereal, s.estado,
               m.fecha_muestreo, a.seccion, a.grado, a.factor, a.tas
        FROM silos s
